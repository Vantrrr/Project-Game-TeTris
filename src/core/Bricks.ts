  ...
  draw() {
    for (let row = 0; row < this.layout[this.activeIndex].length; row++) {
      for (
        ...
        if (
          this.layout[this.activeIndex][row][col] !== this.game.WHITE_COLOR_ID
        ) {
          this.board.drawCell(
            col + this.colPos,
            row + this.rowPos,
            this.game.COLOR_MAPPING[this.id]
          );
        }
      }
    }
      ...
      for (let row = 0; row < this.layout[this.activeIndex].length; row++) {
        for (
          ...
          if (
            ...
            this.board.grid[row + this.rowPos][col + this.colPos] =
              this.game.COLOR_MAPPING[this.id];
          }
        }
      }
